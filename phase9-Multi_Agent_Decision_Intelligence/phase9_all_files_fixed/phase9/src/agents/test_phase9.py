...
def test_finance_agent_roi_is_net_return_not_gross_multiple():
    """Regression test tied to the Phase 6 audit fix (engine.py
    compute_metrics), independently re-checked here because src/finance/
    was pulled into Phase 9 as a byte-identical copy of Phase 6's files and
    could silently regress to the pre-fix formula on a future re-copy.
    roi_total_holding_period surfaces in FinanceAgent.metrics even though
    the Decision Policy itself does not use it -- so it must still be
    correct for any downstream consumer (e.g. Phase 10) reading it there.
    For a single-year holding period, ROI must equal IRR (same cash-flow
    pair, same definition) -- NOT offset by ~1.0 (100 percentage points),
    which is exactly the signature the old bug left."""
    f = get_financials(1, annual_operating_expenses=180000, vacancy_rate=0.05,
                        appreciation_rate=0.08, holding_period_years=1)
    assert f.status == "PASS"
    assert abs(f.metrics["roi_total_holding_period"] - f.metrics["irr"]) < 0.001, (
        f"single-period ROI ({f.metrics['roi_total_holding_period']}) should equal "
        f"IRR ({f.metrics['irr']})"
    )
