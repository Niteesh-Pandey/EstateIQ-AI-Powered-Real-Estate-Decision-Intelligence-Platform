"""
Phase 8, Section 8.2 -- Geo Data Loader
===========================================
Sources every OBSERVED geo input directly from Phase 1's validated tables.
Same DB/CSV dual-path convention as Phase 3/5/6/7: `load_locality_geo_from_db()`
is the production path (live PostgreSQL via db/connection.py);
`load_locality_geo_from_csv()` is the documented dev/test-only path reading
the identical rows from data/processed/*.csv.

What this dataset does NOT have, and this module therefore never invents
...
TODAY = pd.Timestamp("2026-09-03")  # analysis reference date for planned-vs-completed infra classification
