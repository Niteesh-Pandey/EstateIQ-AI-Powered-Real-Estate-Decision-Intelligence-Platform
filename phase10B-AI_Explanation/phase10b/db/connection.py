...
Required env vars (see .env.example -- copy it to .env and fill in real
values; python-dotenv loads .env automatically on import, no manual
`export` needed):
...
# Loads variables from a `.env` file (in this file's parent directory, i.e.
# the project root) into the process environment. Safe to call even if no
# .env exists yet -- it's a no-op in that case, and real environment
# variables (e.g. set by a deployment platform) always take priority over
# anything in .env (override=False is the default).
